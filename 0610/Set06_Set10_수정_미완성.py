...
data6.columns[data6.dtypes =='object']
...
sel_var_list=lm2.pvalues.index[lm2.pvalues < 0.05].drop('Intercept')

len(sel_var_list)  # 13

(lm2.params[sel_var_list] < 0).sum()

(lm2.outlier_test() ['bonf(p)']< 0.05).sum()
data6[lm2.outlier_test() ['bonf(p)']< 0.05]
...
q3=pd.DataFrame(q3, columns=['State', 'mape'])

q3.iloc[q3.mape.argmin(),:]
...
drop_list=data10.columns[data10.isna().all(axis=0)]
data10=data10.drop(columns=drop_list)
