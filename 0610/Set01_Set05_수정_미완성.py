...
q2.nlargest(2)  # 상위 n번째 값과 인덱스명 추출
q2.nlargest(2).index
...
# 내림
# np.floor(값*1000)/1000   => 정수형으로 리턴
# np.ceil(값*1000)/1000    => 정수형으로 리턴
...
lm1=LinearRegression().fit(q3[var_list], q3['Sales'])
# 주의점: 결측치가 없어야 됨, 입력값(X변수들)은 데이터 구조가 2차

dir(lm1)

q3_out=pd.Series(lm1.coef_, index=var_list).sort_values(ascending=False)
...
len(chi2_out[chi2_out.pvalue < 0.05])

# H0[귀무가설] : 두변수가 독립이다
# H1[대립가설] : 두변수가 독립이 아니다(상관이 있다)
...
chi2_out[chi2_out.pvalue < 0.05]['pvalue'].max()
...
# 변수 생성(문자열 -> 수치화), 결측치 제거 후 사용
q3=data2.copy()

# - Sex는 M을 0, F를 1로 변환하여 Sex_cd 변수 생성
q3['Sex_cd'] = np.where(q3.Sex == 'M', 0, 1)

# q3['Sex_cd'] = np.where(q3.Sex.isna(),  q3.Sex, 
#                         np.where(q3.Sex == 'M', 0, 1))
...
# - Age, Na_to_k, Sex_cd, BP_cd, Ch_cd를 Feature로, Drug을 Label로
...
from scipy.stats import ttest_ind, bartlett, levene  # 등분산 검정용

q2_f=q1[q1.gender == 'Female']['forehead_ratio']
q2_m=q1[q1.gender == 'Male']['forehead_ratio']

bartlett(q2_f, q2_m).pvalue < 0.05
...
# H0: 두집단의 평균은 같다
# H1: 두집단의 평균은 같지 않다
# 답: 2.999, Y(남여간의 이마 비율의 평균이 다르다는 의미)
...
precision_score(test['gender'], pred_class, pos_label='Male')
...
print(classification_report(test['gender'], pred_class2))
